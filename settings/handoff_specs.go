...
func HandoffSpecs() []Spec {
	return []Spec{
		// ── 主开关 ──────────────────────────────────────────────────
		{
			...
			Default:         true,
			Description:     "启用会话交接",
			DescriptionLong: "主开关：开启后，当会话上下文达到阈值时自动执行 handoff，生成摘要并提示新会话。关闭后完全不触发自动交接，仅保留手动 /handoff skill 入口。",
			Unit:            "",
			HotReload:       true,
			DangerLevel:     Warning,
		},
		...
		// ── 触发模式（Phase 2 新增） ─────────────────────────────────
		{
			...
			DescriptionLong: "auto=按阈值自动触发；manual=仅响应用户 /handoff 命令；hybrid=阈值 + 用户命令均可触发。hybrid 模式下阈值触发也会注入 /handoff skill 提示。",
			Unit:            "",
			HotReload:       true,
			DangerLevel:     Warning,
		},
