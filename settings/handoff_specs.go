...
func HandoffSpecs() []Spec {
	return []Spec{
		// ── 主开关 ──────────────────────────────────────────────────
		{
			...
			Default:         false,
			Description:     "启用会话交接",
			DescriptionLong: "主开关：开启后，网关检测上下文阈值；仅显式客户端模式或用户 /handoff 命令返回 202 resume_packet。网关不会改写发送给 LLM 的请求体。",
			Unit:            "",
			HotReload:       true,
			DangerLevel:     Warning,
		},
...

		{
			...
			Description:     "客户端交接模式",
			DescriptionLong: "transparent=默认兼容模式，不改写请求体且自动阈值请求继续转发；explicit=返回 202 和 resume_packet，由客户端创建新会话。单次请求可用 X-Gw-Handoff-Mode: explicit 覆盖租户默认模式。",
			HotReload:       true,
			DangerLevel:     Warning,
		},
		...
		// ── Token 绝对阈值 ───────────────────────────────────────────
		{
			...
			DescriptionLong: "当会话累计 token 数达到此阈值时触发 handoff（默认 300K；2026-09-22 Wave 2 自 180K 上调，以设计文档最后出现值为准）。0 表示禁用绝对阈值，仅依赖百分比阈值。",
			Unit:            "tokens",
			HotReload:       true,
			DangerLevel:     Safe,
		},
