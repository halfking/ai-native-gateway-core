	...
	return []*Spec{
		{
			...
			Default:         "smart",
			Description:     "压缩模式",
			DescriptionLong: "off=从不压缩；auto_threshold=请求前按阈值压缩；on_4xx=仅在 upstream 返回 context_length_exceeded 后压缩；smart=v4 智能压缩（默认，含工具裁剪+滑动窗口）；aggressive=v4 激进压缩（含任务分析+主动压缩）；headroom=Headroom 算法压缩（智能去冗余+自适应调整）。",
			Unit:            "",
			DangerLevel:     Warning,
			HotReload:       true,
		...
		},
		{
			...
			Max:             &maxRatio,
			Default:         0.5,
			Description:     "Headroom 目标压缩率",
			DescriptionLong: "Headroom 模式的目标压缩比例（0.1-0.9）。0.5 表示压缩到原大小的 50%。",
			Unit:            "比例",
			DangerLevel:     Warning,
			HotReload:       true,
			Observability:   "/api/admin/compression/stats",
		},
		{
			...
			Default:         true,
			Description:     "启用智能去冗余",
			DescriptionLong: "Headroom 模式：启用 SmartCrusher 去除冗余词汇和空白。",
			Unit:            "",
			DangerLevel:     Warning,
			HotReload:       true,
			Observability:   "/api/admin/compression/stats",
		},
		{
			...
			Default:         true,
			Description:     "启用自适应调整",
			DescriptionLong: "Headroom 模式：启用 AdaptiveSizer 动态调整内容长度。",
			Unit:            "",
			DangerLevel:     Warning,
			HotReload:       true,
			Observability:   "/api/admin/compression/stats",
