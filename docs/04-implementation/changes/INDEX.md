...
- [2026-08-26 · v6 请求类型落库（request_class/due_at，migration 608）](2026-08-26-v6-request-class-persistence.md) — request_logs 全链路持久化（INSERT $101/$102 + UPDATE $98/$99 + upsert 防回退 + admin 过滤/展示 + fsstore 同构镜像）；顺带恢复 d2cbaf88b 剥离的 POST /providers/{id}/models 405 修复、修复 fsstore GetRequest 索引分词/时区 bug、隔离四个腐烂 admin 测试恢复包可编译；离线 SQL 契约测试钉死占位符对齐
