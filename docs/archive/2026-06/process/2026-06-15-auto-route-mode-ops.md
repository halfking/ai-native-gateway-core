---
archived_from: docs/2026-06-15-auto-route-mode-ops.md
...
```bash
# Healthz
curl -i https://__DOMAIN_8__/healthz

# Auto route — code task
curl -i -X POST https://__DOMAIN_8__/v1/chat/completions \
...
curl -is -X POST https://__DOMAIN_8__/v1/chat/completions \
  -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
...
```bash
# Recent decisions
curl -s "https://__DOMAIN_8__/api/admin/auto-route/decisions?limit=10" \
  -H "Authorization: Bearer $ADMIN_TOKEN" | jq

# Current index snapshot
curl -s "https://__DOMAIN_8__/api/admin/auto-route/index?top=20" \
  -H "Authorization: Bearer $ADMIN_TOKEN" | jq

# Set sticky profile for an API key
curl -s -X PUT "https://__DOMAIN_8__/api/admin/auto-route/profile" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"api_key_id": 42, "profile": "cost_first"}' | jq

# Aggregated stats
curl -s "https://__DOMAIN_8__/api/admin/auto-route/audit" \
  -H "Authorization: Bearer $ADMIN_TOKEN" | jq

# Manually trigger index refresh
curl -s -X POST "https://__DOMAIN_8__/api/admin/auto-route/refresh" \
  -H "Authorization: Bearer $ADMIN_TOKEN" | jq
```
...
curl -s "https://__DOMAIN_8__/api/admin/auto-route/index?top=1" \
