...
| 任务 | 执行时间 | 优先级 | 状态 | 文档 |
|------|---------|--------|------|------|
| 1小时监控检查 | 2026-07-19 17:05 | P0 | 待执行 | [01-1小时后监控检查.md](./_templates/01-1小时后监控检查.md) |
| 6小时效果评估 | 2026-07-19 22:00 | P0 | 待执行 | [02-6小时效果评估.md](./_templates/02-6小时效果评估.md) |
| 24小时完整分析 | 2026-07-20 16:00 | P0 | 待执行 | [03-24小时完整分析.md](./_templates/03-24小时完整分析.md) |
| 1周长期趋势分析 | 2026-07-26 16:00 | P0 | 待执行 | [04-1周长期趋势分析.md](./_templates/04-1周长期趋势分析.md) |

## 归档规则

- 本目录只保留当前待执行任务、任务索引和可复用模板。
- 任务完成后 7 天内迁出本目录，并使用主题归档目录保存：`docs/omnifree/`、`docs/audit/`、`docs/deploy/`、`docs/incidents/` 等。
- 归档文件优先使用 `YYYY-MM-DD-<topic>.md` 命名；已有稳定名称的文档迁移时保留原名，避免无必要的链接变更。
- 完成文档不得长期留在“待处理任务”目录；请在 `任务清单.md` 中保留归档交叉引用。
- `01/02/03/04` 时间线文件位于 `_templates/`，复制使用时先填写触发条件、负责人和起算时间。
...
请执行 docs/待处理任务/_templates/01-1小时后监控检查.md 中的所有步骤。
...
请执行 docs/待处理任务/_templates/02-6小时效果评估.md 中的所有步骤。
```
...
请执行 docs/待处理任务/_templates/03-24小时完整分析.md 中的所有步骤。
```
...
请执行 docs/待处理任务/_templates/04-1周长期趋势分析.md 中的所有步骤，并给出 P1/P2 优化建议。
```
