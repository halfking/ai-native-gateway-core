...
- [x] 登录功能正常（admin / __REDACTED_SSH_PASSWORD__）
...
curl -s http://154.host:8781/api/auth/token -X POST -H "Content-Type: application/json" -d '{"username":"admin","password":"__REDACTED_SSH_PASSWORD__"}' | jq .
```
