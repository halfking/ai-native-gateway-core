...
### 前端测试（浏览器）
```javascript
// 1. 打开 https://__DOMAIN_8__/routing-v2/credentials
// 2. 点击任一凭据
// 3. 在控制台执行：
...
```bash
# 测试决策API
curl -s "https://__DOMAIN_8__/api/credentials/decisions?credential_id=1&limit=5" | jq
...
# 3. 构建镜像
docker build -t __DOMAIN_9__/kx-llm-gateway-go:$(date +%Y%m%d) .

# 4. 推送镜像
docker push __DOMAIN_9__/kx-llm-gateway-go:$(date +%Y%m%d)

# 5. 部署184
kubectl -n pms-test set image deployment/kx-llm-gateway-go \
  llm-gateway-go=__DOMAIN_9__/kx-llm-gateway-go:$(date +%Y%m%d)

# 6. 验证
kubectl -n pms-test rollout status deployment/kx-llm-gateway-go
curl -s https://__DOMAIN_8__/healthz | jq
```
...
# 3. 检查API可达性
curl -v https://__DOMAIN_8__/api/credentials/decisions?credential_id=1
```
...
```bash
# 1. 检查凭据是否存在
curl "https://__DOMAIN_8__/api/credentials/monitor-summary" | jq '.credentials[] | select(.id==123)'
