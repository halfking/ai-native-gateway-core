...
| SC-09 | P0 | 按错误类型选择探测起点 | transport/transient 使用 5s → 15s → 30s → 60s（封顶 60s）；quota 固定 2m/5m；rate_limit 不快于策略下限 3m |
| SC-10 | P0 | 连续失败与恢复后的退避 | credential cooling 使用 30s → 2m → 5m，ConsecutiveFails 封顶 10；成功探测将失败计数清零，节点 transport 链按 attempt 饱和于 60s |
