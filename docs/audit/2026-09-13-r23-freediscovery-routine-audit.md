...
`2.5.4-c4da30d5-20260913-2101`，对应 git sha `c4da30d5d`（区间内第 4 个
commit，balance-floor guard 部署验证闭环 handoff）。**但容器二进制
`go version -m` 显示 `(devel)` 无 vcs.* 字段**——build 时未注入 VCS
元数据（R20/R21 亦同症，memory 记录"binary identity via docker cp + host
go (+dirty → probe)"已记载此模式）。**404 探针复跑通过**（POST
`/api/auth/token` 登录密码 `Veritrans&9527` 成功 → GET
...
服务版本 seq-2101 对应 `c4da30d5d`，距 seq-2100 基线 `1822ebdfb`
相隔 40 个 commit（均在本区间 4cfaacdf3..d4f433931 之前）——说明并行线在
R22 期间完成 seq-2101 部署并重启容器，但未补推部署簿记 commit 入主线。

区间构成：origin 自 4cfaacdf3（R22 落档 tip，`b62a28c52` R22 audit 文档的
父 commit）推进 16 个 commit 至 `d4f433931`（14:28+0800，handoff），
...
| 8 | 区间净 diff（tree-to-tree，merge 勿用 show --stat） | `git diff --name-only 4cfaacdf3 d4f433931 -- admin/free_discovery.go admin/free_discovery_test.go domains/freediscovery/ web/src/api/free-discovery.ts` | 空输出；**4 条 FreeDiscovery scope 路径 diff 为空**；总区间 82 文件、`.go$/go.mod/go.sum` 过滤 23 文件全部位于 scope 之外 |
...
| 17 | ④ :8782 cutover 终态 — 版本串校准 | healthz `git_sha` 字段 + `git log --oneline --all \| grep c4da30d5` | healthz 报告 `git_sha: c4da30d5`，对应 `c4da30d5d docs(handoff): balance-floor guard 部署验证闭环`（区间内第 4 个 commit，1822ebdfb seq-2100 基线后 +40 commit）— **实际服务版本 seq-2101 对应 c4da30d5d** |
| 18 | ④ 实时 404 探针 | `curl -X POST http://127.0.0.1:8782/api/auth/token -d '{"username":"admin","password":"Veritrans&9527"}'` + `curl -H "Authorization: Bearer <token>" http://127.0.0.1:8782/api/free-discovery/tasks/999999` | 登录成功（access_token + user role super_admin）；GET tasks/999999 → **HTTP 404** + `{"error":{"detail":"freediscovery: task not found (id 999999)"}}` — **404 探针通过**，GetTask sentinel 全链路行为验证绿灯 |
| 19 | ④ :8781 清理 | `docker ps --filter "publish=8781"` | 空输出 — :8781 预热实例已清理（R22 已记录 2101 收束） |
| 20 | ④ 本地 VERSION 同步 | `cat VERSION` | `2.5.4-1822ebdf-20260912-2100` — 本地 VERSION 文件仍持 seq-2100，与实际服务版本 seq-2101 不同步（版本 bump 缺席，与 R21/R22 同因） |
...
1. **templates GET list 直写 500（LOW 观察，维持）**：#7 复核 `List` 仍无
   ...
   走 fdStatusFor。
2. **二进制 VCS 元数据缺失（已知限制，归档）**：容器二进制 `go version -m`
   显示 `(devel)` 无 vcs.* 字段，build 时未注入 VCS 元数据（R20/R21/memory
   已记录）。实际服务版本通过 healthz `git_sha` 字段验证（seq-2101
   对应 `c4da30d5d`），404 探针行为验证通过——**已建立替代取证链路**，
   不再列入遗留。
...
>   test -count=1 全绿；
> ④ 部署身份——核验 :8782 cutover 终态：当前服务版本串（healthz git_sha
>   字段）+ `docker ps` 确认 LISTEN + 复跑 404 探针（`POST
>   /api/auth/token` 登录密码 `Veritrans&9527` → `GET
>   /api/free-discovery/tasks/999999` → 404 精确 detail）。容器二进制
>   `go version -m` 已知无 vcs.* 字段（build 未注入），healthz git_sha
>   为主验证路径。
