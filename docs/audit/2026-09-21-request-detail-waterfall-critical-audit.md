...
> 请求详情瀑布任务已经在 active `8f6af633/#2173` 完成实际验收；先阅读本文件，**不要重复部署或重新实现 waterfall UI**。下一轮优先处理两项独立风险：（1）按 migration 的 applied/pending 状态、目标库 marker 与 controlled replay 设计 11 个 checksum mismatch + 662/663 stale marker 的双 provenance 治理，禁止直接覆盖 `docs/db-changelog.md` hash 刷绿；（2）在干净 macOS 临时目录/高并发条件下调查 `cmd/gateway.TestLiteRequestLogSink_TurnNoContinuesAcrossRestart` 的 `MkdirAll invalid argument` 组合不稳定性，精确用例虽 `-count=100` PASS，仍不可声称全仓 Go 绿。继续遵守共享工作区纪律：不要暂存 `docs/audit/252-pg-log-audit-2026-09-19/pg_252_pg17_logs.txt` 或其他未跟踪原始日志，也不要接管并行会话的版本、台账和 menu 生成物。
