...
**客户端接入面普查(2026-09-16 02:50 补记,request_logs_2026_09 只读聚合)**:

- 生产整体流量**并不稀疏**:近 7 天 **181,049 条**请求、13 个活跃 key,时段集中在白天(末次 09-15 18:41)——稀疏的只是 auto:同窗口 `client_model='auto'` **精确 0 条**。
- 流量头部(显式指定模型):#105 deploy-smoke-245(78,831 条,名义冒烟实为评测/回放量级)、#3(24,906)、#6(5,141)、#86(4,431)、#114 openpocket(379);另有 65,872 条 api_key_id 为空(日志链路侧缺口,未计入 key 分布,建议另登)。
- **定性升级:T1 样本不足是结构性缺失而非时间性等待**——没有任何生产客户端配置 model=auto,首周窗口(乃至无限期)都不可能自然凑够 ≥30 条;09-23 收口几乎必然落入"无统计基础"分支。
- **给人工的决策项(可立即启动,不必等收口)**:①业务侧确认 #3/#6/#86/openpocket 等真实业务 key 哪些可容忍路由代选,将其客户端 model 参数切到 auto(生产接入面即建立);②若短期无业务 key 可切,#105 的评测/回放流(78k/7d)天然是受控合成流量源,可决策是否以其承担 auto 采样面(需评审其对 selection/tuning 数据的污染面)——两条路任选其一,T1 才有统计基础。
...
- **催办记录(2026-09-16 02:36 立案,02:20 生产复核仍 16 行全空)**:
  - **催办对象**:运营/商务侧,涉及 suyun#39(6 行 claude 绑定)、apinext#61(4 行,含 claude/ 斜杠别名)、智码#35(5 行);共 16 行。
  - **所需动作**:为全部 16 行 claude 系绑定填写 `unit_price_in_per_1m` / `unit_price_out_per_1m` / `pricing_source` / `pricing_updated_at`(含 claude/sonnet-5、claude/opus-5 斜杠别名行,勿漏)。
  - **阻塞后果**:G4 成本均衡判定无法重跑(重跑只会复现"无单价通道排除"旧边界);三通道在 code/reasoning primary 面承担真实流量,成本核算与候选资格终审双盲。
  - **截止时间:2026-09-19 18:00**(为 09-23 首周收口预留 3 天余量;逾期则 T3 在首周收口时定格为"补录催办未果,不可验证分支成立",并升级至人工层面跟进)。
  - **复核方式**:每日任意会话可跑 §附-4 SQL 验证;首周收口(09-23 定时任务第 7 次)必核一次,补录已发生则按 round2-plan 流程重跑 G4 并更新 N6。
...
| T3 Claude-5 单价 | **补录未发生** → 不可验证标注已落 list-v2 N6 | 催办已立案(§三,截止 2026-09-19 18:00);补录后重跑 G4 |
