...
- 使用 migration 622 新增的单调 `aggregation_id`，不再错误依赖来源表声明为非唯一的 legacy `id`。
...
- `sql/migrations/startup/622_provider_error_aggregator_state.sql`
- `sql/migrations/startup/622_provider_error_aggregator_state.down.sql`
...
1. 按顺序执行 migration 620、621、622；migration 616 已应用，不应直接重写其 up 文件。
2. 生产执行 migration 622 前应备份 `candidate_failure_logs_hot`，并观察 `aggregation_id` 回填耗时。
