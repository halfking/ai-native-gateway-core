...
> 生成日期：2026-08-15。代码基线：go-3 HEAD `f249c4f4`（代码事实）+ 修订0811 main@`96bc1425f`（文档结论）。
...
1. **以代码为真相**：任何「实现状态」论断以当前代码基线（go-3 f249c4f4）为准，文档说法仅是历史记录。
...
| 锚点 | 值 | 用途 |
|---|---|---|
| 代码事实基线 | go-3 `f249c4f4`（OBS-BE3 合并后） | 「已实现/未接线」判定的唯一依据 |
| 文档历史基线 | 修订0811 main@`96bc1425f` | M0/M1/M2 完成的官方记录 |
| 最近完成节点 | 修订0811/`24-M2收尾`（08-15） | P2-C1 信封适配 + P2-S1 KeepaliveSender 删除 |
| 下一实施节点 | **M3 durable** | EncryptWithAAD → durable_llm_tasks → PG 回源 → fencing → RecoveryWorker |

> ⚠️ 工作区差异：go-3 含 OBS-BE1 liveactions + web 新组件；official-deploy 含 24-M2 收尾文档。实施计划按两者合并视角编写。
