...
| **V2** | **企业级多租户凭据治理** | 不同客户、不同部门、不同项目共用同一网关但凭据、限速、配额、审计必须严格隔离 | URSM v2 为默认 authoritative 的 tenant-aware 路径（Redis/Lua 实时 + PG durable），与 legacy / canary / fallback backend 并存（收口见 V6-W1）+ `domains/credentialstate/` + `pool/` + `resolve/` + `credentialfpslot/` | [`routing-and-state.md`](../03-design/01-architecture/architecture/routing-and-state.md)、[`omniroute-integration-boundary.md`](../03-design/01-architecture/architecture/omniroute-integration-boundary.md) |
...
| Agent 协议 | 实验 | 实验 | **plugin-runtime（已有）+ MCP stdio/HTTP/SSE（计划，V6-W4）+ A2A message/send 试点（计划）+ Fusion（设计）** |
| 合规 / 审计 | 通用 SaaS 日志 | 边缘日志 | **armor PII 判别（已有）+ Presidio sidecar（计划，V6-W3）+ tenant RLS（部分表已有，Maintain/ASM 矩阵未实证）+ ASM 投影 + Maintain license** |
...
- **会话维度**：Session 详情 + 对比 + 会话健康（`ops/session-health-operations.md`，**[待建]**：`docs/runbooks/` 下尚未落盘，现有 runbook 见 `docs/runbooks/` 目录）。
