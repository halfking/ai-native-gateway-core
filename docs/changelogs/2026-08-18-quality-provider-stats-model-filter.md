# 2026-08-18 — 品质统计支持按模型粒度过滤（方案 C）+ 抽屉"模型30天统计"块

> 在供应商级统计（方案 A）之上，为品质统计增加模型级过滤维度，
...
3. 无 SQL/索引变更（复用 `usage_ledger_hot` 既有聚合）。
...
| `web/src/components/RequestLogDrawer.vue` | 修改 | 新增模型级统计块（方案 A 供应商级 + 方案 C 模型级） |
| `CHANGELOG.md` + 本文档 | 修改 | rule 36 变更记录 |

## 3. 为什么这样做

- 方案 C 复用同一聚合口径与窗口（30 天、`usage_ledger_hot`），避免重复建表/索引。
...
- 部署为 build 1612（git_sha 96e15c2b，含本轮 WIP commit）；`version.json` 本地已 bump。
- 验证期间服务器版本显示一度为 #1611（另有部署发生），与当前分支一致性待确认。

## 6. 下一步建议

- 提交本轮方案 C 改动（本 changelog 附随），按 rule 35/36 推送。
- 与最近一次他人部署（1611/1612）比对，确认其内容与本分支一致，避免覆盖。