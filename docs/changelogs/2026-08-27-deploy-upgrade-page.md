...
- DNS note: `llm.kxpms.cn` currently resolves to <env:HOST_154_IP> directly, so the "public" path is 154's own vhost; the 252 vhost was verified via `--resolve` and keeps dual-ingress protection valid for future DNS flips back to 252.
