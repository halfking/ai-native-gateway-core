...
async def handle_chat(request: web.Request) -> web.StreamResponse:
    ...
    if mode == "healthy":
        ...
        if random.random() < 0.5:
            STATE.counters["requests_error"] += 1
            return web.json_response({"error": {"message": "Internal error", "type": "server_error"}}, status=500)
        delay_ms = random.randint(STATE.latency_min_ms, STATE.latency_max_ms)
        await asyncio.sleep(delay_ms / 1000.0)
...
async def handle_admin_state_set(request: web.Request) -> web.Response:
    ...
    valid_modes = ["healthy", "slow", "rate_limited", "quota_exceeded", "auth_error", 
                   "server_error", "timeout", "connection_refused", "broken_stream", "flaky"]
...
async def handle_admin_reset(_request: web.Request) -> web.Response:
    STATE.change_mode("healthy", ttl_seconds=0)
    STATE.counters = {"requests_total": 0, "requests_success": 0, "requests_error": 0, "state_changes": 1}
    STATE.save()
...
async def handle_admin_latency(request: web.Request) -> web.Response:
    ...
    STATE.save()
    return web.json_response({"status": "ok", "latency_min_ms": STATE.latency_min_ms, "latency_max_ms": STATE.latency_max_ms})
