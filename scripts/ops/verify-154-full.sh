#!/usr/bin/env bash
# Full verification on 154 after ops/blocklist deploy.
set -euo pipefail
...
echo "=== 154 完整验证 (ops + blocklist + board) ==="
...
def req(method, url, data=None, hdrs=None, timeout=20):
    h = dict(hdrs or {})
    body = None
    if data is not None:
...
env = {}
for ln in Path("/etc/llm-gateway-go/env").read_text().splitlines():
...
t0=time.time()
st, raw = req("GET", base+"/api/admin/dashboard/board?days=1&include_operational=1", hdrs=H)
...
t0=time.time()
st, raw = req("GET", base+"/api/admin/ops/overview", hdrs=H)
ms=int((time.time()-t0)*1000)
if st == 200:
    ov=json.loads(raw)
...
# 5) blocklist CRUD
st, raw = req("GET", base+"/api/admin/security/ip-blocklist", hdrs=H)
if st == 200:
    ok("blocklist list OK")
else:
    fail(f"blocklist list HTTP {st}")

test_ip = "203.0.113.98"
st, raw = req("POST", base+"/api/admin/security/ip-blocklist", {
    "ip_or_cidr": test_ip, "reason": "154-verify-test", "scope": "ops"
}, hdrs=H)
entry_id = None
if st in (200,201):
...
st, raw = req("POST", base+"/api/admin/security/ip-blocklist/reload", hdrs=H)
if st == 200:
...
if entry_id:
    st, _ = req("DELETE", base+f"/api/admin/security/ip-blocklist/{entry_id}", hdrs=H)
    if st in (200,204):
...
if env.get("OPS_NODE_REGION") == "154":
    ok("OPS_NODE_REGION=154")
else:
    warn(f"OPS_NODE_REGION={env.get('"'"'OPS_NODE_REGION'"'"')}")
if env.get("OPS_DATA_DIR"):
    ok(f"OPS_DATA_DIR={env.get('"'"'OPS_DATA_DIR'"'"')}")
else:
    warn("OPS_DATA_DIR missing")
...
# 7) public ops register endpoint (license-authority)
try:
    st, raw = req("POST", "https://llm.kxpms.cn/api/v1/ops/nodes/register", {"license_key": "test"}, timeout=8)
    if st == 404:
        warn("public /api/v1/ops/nodes/register returns 404 (license-authority not routed)")
    elif st in (400, 401, 403, 422):
        ok(f"public register endpoint reachable HTTP {st}")
    else:
        warn(f"public register HTTP {st}")
except Exception as e:
    warn(f"public register check skipped: {e}")

# 8) DB tables via psql
import subprocess
...
echo ""
echo "=== 252 数据面（经 154 查询）==="
bash "$(dirname "$0")/verify-ops-data-plane.sh" 154 2>&1 | sed 's/^/  /'
...
echo ""
echo "=== 公网 HTTPS 冒烟 ==="
HTTP_PUB=$(curl -sS -o /dev/null -w '%{http_code}' --connect-timeout 8 --max-time 12 --resolve 'llm.kxpms.cn:443:47.97.111.154' 'https://llm.kxpms.cn/healthz' 2>/dev/null || echo 'FAIL')
if [[ "$HTTP_PUB" == "200" ]]; then
  ok "https://llm.kxpms.cn/healthz = 200"
else
  warn "https://llm.kxpms.cn/healthz = $HTTP_PUB"
fi
