...
dl_write_env() {
  ...
  fi
  {
    ...
    # 2026-09-18：配合 PG max_connections=1000 落地（ALTER SYSTEM），storage
    # full 模式 PG/Redis pool size 由 .env.local 提供；未设时走 config 默认
    # 200。Honor 显式配置；只有非正值才清空（避免历史空值导致 pool=0）。
