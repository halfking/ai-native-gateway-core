...
_deploy_verify_curl_code() {
  ...
  "$ssh_cmd" "curl -sS -o /dev/null -w '%{http_code}' --max-time 5 '$url'" 2>/dev/null || echo "000"
}
...
deploy_verify_gateway_ready() {
  local ssh_cmd=$1 service_name=$2 port=${3:-8781} timeout_s=${4:-120}
  local base="http://127.0.0.1:${port}"
  local deadline=$(( $(date +%s) + timeout_s ))
  local last_bg="000"

  _deploy_verify_log "等待 DB 就绪 (最长 ${timeout_s}s，EnsureSchema 可能需 60-90s)..."

  while (( $(date +%s) < deadline )); do
    local pg_disabled
    pg_disabled=$("$ssh_cmd" "journalctl -u '$service_name' --since '3 minutes ago' --no-pager -o cat 2>/dev/null | grep -c 'postgres disabled' || true" | tr -d '[:space:]')
    pg_disabled=${pg_disabled:-0}
    if [[ "$pg_disabled" -gt 0 ]]; then
      _deploy_verify_err "postgres disabled（${pg_disabled} 条日志）"
      "$ssh_cmd" "journalctl -u '$service_name' --since '3 minutes ago' --no-pager | grep 'postgres disabled' | tail -3" || true
    ...
    if [[ "$last_bg" == "401" || "$last_bg" == "200" ]]; then
      ...
      if [[ "$health" == "200" ]]; then
        _deploy_verify_log "✓ DB 就绪 (background-tasks=${last_bg}, healthz=200)"
        return 0
      fi
    fi

    if [[ "$last_bg" == "503" ]]; then
      _deploy_verify_log "… 仍在启动 (background-tasks=503)，继续等待"
    fi
    sleep 4
  done
...
deploy_preflight_pg_from_remote_env() {
  ...
  if ! "$ssh_cmd" "set -e; ENV='$env_file'; test -f \"\$ENV\"; DB=\$(grep '^LLM_GATEWAY_DATABASE_URL=' \"\$ENV\" | cut -d= -f2-); test -n \"\$DB\"; psql \"\$DB\" -c 'SELECT 1' >/dev/null"; then
