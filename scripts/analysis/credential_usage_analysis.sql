...
credential_stats AS (
  SELECT 
    ...
    -- 模型分布（Top 3）
    (
      SELECT STRING_AGG(model_usage, ', ' ORDER BY request_count DESC)
      FROM (
        SELECT 
          rl2.outbound_model || ' (' || COUNT(*) || ')' AS model_usage,
          COUNT(*) AS request_count
        FROM request_logs rl2, time_range tr
        WHERE rl2.credential_id = rl.credential_id
          AND rl2.ts BETWEEN tr.start_time AND tr.end_time
          AND rl2.outbound_model IS NOT NULL
        GROUP BY rl2.outbound_model
        ORDER BY COUNT(*) DESC
        LIMIT 3
      ) top_models
    ) AS top_models
