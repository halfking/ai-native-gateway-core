...
func (h *Handler) RegisterRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("/api/routing/policy", admin(h.handleRoutingPolicy))
	mux.HandleFunc("/api/routing/featured", admin(h.handleRoutingFeatured))
	...
	mux.HandleFunc("/api/routing/audit", admin(h.handleRoutingAudit))
	mux.HandleFunc("/api/routing/probe", admin(h.handleRoutingProbe))
	mux.HandleFunc("/api/routing/manual-priority", admin(h.handleRoutingManualPriority))
	mux.HandleFunc("/api/routing/score-details", admin(h.handleRoutingScoreDetails))
	mux.HandleFunc("/api/routing/scoring-weights", admin(h.handleRoutingScoringWeights))
	...
	mux.HandleFunc("/api/providers", admin(h.handleProvidersRoot))
	mux.HandleFunc("/api/providers/", admin(h.handleProviders))
	mux.HandleFunc("/api/providers/seed-from-catalog", admin(h.handleSeedFromCatalog))
	mux.HandleFunc("/api/providers/credentials/", admin(h.handleForceRecover))
	...
	mux.HandleFunc("/api/catalog", admin(h.handleCatalogRoot))
	...
	mux.HandleFunc("/api/free-pool/models", admin(h.handleFreePoolModels))
	mux.HandleFunc("/api/free-pool/catalog", admin(h.handleFreePoolCatalog))
	mux.HandleFunc("/api/free-pool/available", admin(h.handleFreePoolAvailable))
	mux.HandleFunc("/api/free-pool/register-all", admin(h.handleFreePoolRegisterAll))
	mux.HandleFunc("/api/free-pool/import-env", admin(h.handleFreePoolImportEnv))
	mux.HandleFunc("/api/free-pool/bootstrap", admin(h.handleFreePoolBootstrap))
	mux.HandleFunc("/api/free-pool/bridge-oauth", admin(h.handleFreePoolBridgeOAuth))
	mux.HandleFunc("/api/free-pool/discover", admin(h.handleFreePoolDiscover))
	mux.HandleFunc("/api/free-pool/bulk-register", admin(h.handleFreePoolBulkRegister))
	mux.HandleFunc("/api/free-pool/methods", admin(h.handleFreePoolMethods))
	mux.HandleFunc("/api/free-pool/discovery-status", admin(h.handleFreePoolDiscoveryStatus))
	mux.HandleFunc("/api/free-pool/signup-hub", admin(h.handleFreePoolSignupHub))
	mux.HandleFunc("/api/free-pool/temp-email", admin(h.handleFreePoolTempEmail))
	mux.HandleFunc("/api/free-pool/temp-email/poll", admin(h.handleFreePoolTempEmailPoll))
	mux.HandleFunc("/api/free-pool/probe", admin(h.handleFreePoolProbe))
	mux.HandleFunc("/api/free-pool/quick-entry", admin(h.handleFreePoolQuickEntry))
