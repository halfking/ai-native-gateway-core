...
func (h *Handler) getLog(w http.ResponseWriter, r *http.Request) {
	...
	// 2026-07-06: 使用视图查询，避免遗漏 hot 表数据（migration 341）
	// 2026-07-23 BUGFIX: request_logs and request_logs_bodies use the composite
	// (request_id, ts) key; both values must match to avoid attaching a body
	// from a different row when a request ID is reused or legacy rows drift.
