...
import { getTaskAnalyticsDetail, type TaskAnalyticsDetail } from '../api/admin'
...
async function load() {
  ...
  try {
    data.value = await getTaskAnalyticsDetail(taskId.value, days.value)
    renderChart()
  } catch (e: any) {
    console.error('Failed to load task analytics:', e)
    errorMessage.value = e.message || t('sessions.taskAnalytics.loadError')
  } finally {
    loading.value = false
  }
}
...
function renderChart() {
  ...
  const option: EChartsOption = {
    ...
    legend: {
      data: [t('sessions.taskAnalytics.cost'), t('sessions.taskAnalytics.sessions')]
    },
    xAxis: {
      type: 'category',
      data: dates,
    ...
    yAxis: [
      {
        type: 'value',
        name: t('sessions.taskAnalytics.cost'),
        position: 'left',
        axisLabel: { formatter: '${value}' }
      },
      {
        type: 'value',
        name: t('sessions.taskAnalytics.sessions'),
        position: 'right'
      }
    ],
    series: [
...
<template>
  <div v-loading="loading" class="task-analytics-view">
    ...
    <div v-if="data">
      ...
      <el-row :gutter="16" style="margin-bottom: 20px;">
        <el-col :span="6">
          <el-card shadow="hover">
            <div class="stat-item">
              <div class="stat-label">{{ t('sessions.taskAnalytics.totalSessions') }}</div>
        ...
        <el-col :span="6">
          <el-card shadow="hover">
            <div class="stat-item">
              <div class="stat-label">{{ t('sessions.taskAnalytics.totalCost') }}</div>
        ...
        <el-col :span="6">
          <el-card shadow="hover">
            <div class="stat-item">
              <div class="stat-label">{{ t('sessions.taskAnalytics.avgHealth') }}</div>
      ...
      <el-card shadow="hover" style="margin-bottom: 20px;">
        <template #header>
          <span>{{ t('sessions.taskAnalytics.costTrend') }}</span>
        </template>
      ...
      <el-row :gutter="16">
        <el-col :span="12">
          <el-card shadow="hover">
            <template #header>
              <span>{{ t('sessions.taskAnalytics.relatedClients') }}</span>
            </template>
            <el-table :data="data?.related_clients || []" style="width: 100%" max-height="400">
              ...
              <el-table-column prop="total_cost_usd" :label="t('sessions.stats.totalCost')" width="100" align="right">
                <template #default="scope">
                  ${{ (scope?.row?.total_cost_usd ?? 0).toFixed(2) }}
                </template>
              </el-table-column>
        ...
        <el-col :span="12">
          <el-card shadow="hover">
            <template #header>
              <span>{{ t('sessions.taskAnalytics.recentSessions') }}</span>
            </template>
            <el-table :data="data?.recent_sessions || []" style="width: 100%" max-height="400">
              <el-table-column prop="session_id" :label="t('sessions.taskAnalytics.sessionId')" min-width="150">
                <template #default="scope">
              ...
              <el-table-column prop="request_count" :label="t('sessions.taskAnalytics.requests')" width="80" align="right" />
              <el-table-column prop="cost_usd" :label="t('sessions.taskAnalytics.cost')" width="100" align="right">
                <template #default="scope">
                  ${{ (scope?.row?.cost_usd ?? 0).toFixed(4) }}
                </template>
              </el-table-column>
              <el-table-column prop="health_grade" :label="t('sessions.taskAnalytics.health')" width="80" align="center">
                <template #default="scope">
