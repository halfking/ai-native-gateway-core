  ...
  <PublicPortalLayout>
    <div class="sup-page">
      ...
      <el-form label-position="top" class="sup-form">
        ...
        <el-form-item :label="t('public.support.payChannel')">
          <el-radio-group v-model="channel">
            <el-radio value="alipay">{{ t('public.support.alipay') }}</el-radio>
            <el-radio value="wechat">{{ t('public.support.wechat') }}</el-radio>
          </el-radio-group>
        </el-form-item>
      </el-form>
      ...
      <div class="sup-actions">
        <el-button type="primary" size="large" :loading="loading" @click="donate">
          {{ t('public.support.donateBtn') }} · ¥{{ (amountCents / 100).toFixed(2) }}
        </el-button>
      ...
      <el-card v-if="orderNo" shadow="never" class="sup-payment">
        ...
        <template v-if="stubMode">
          ...
          <el-button link @click="router.push('/download')">{{ t('public.support.goDownload') }}</el-button>
        </template>
      </el-card>
    </div>
  </PublicPortalLayout>
...
.sup-payment { margin-top: 1.5rem; }
.sup-qr { max-width: 200px; margin-top: 0.5rem; }
