...
// ProbeHealthPanel — /probe-health body extracted (2026-09-07) so the
// credential monitor page can embed it as the「探测健康」tab. The standalone
// /probe-health route redirects here via /routing-v2/credentials?tab=probe-health.
// `embedded` hides the page header (title/back-link) and outer padding.
...
<template>
  <div class="page-container" :class="{ embedded: props.embedded }">
    <!-- Header (standalone only; the tab host renders its own tabs) -->
    <div class="top-bar" v-if="!props.embedded">
    ...
    <div v-else class="embedded-toolbar">
      <span class="embedded-title">{{ t('probeHealth.title') }}</span>
      <div class="spacer"></div>
      <label class="auto-refresh-label">
        <input type="checkbox" v-model="autoRefresh" />
        {{ t('probeHealth.autoRefresh') }}
