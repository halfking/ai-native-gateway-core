<template>
  <div class="dashboard-stats-row" :class="{ 'dashboard-stats-row--loading': props.loading }">
    <el-row :gutter="16">
      ...
      <el-col v-if="props.showTokens" :xs="24" :sm="12" :md="8" :lg="6" :xl="3">
        <el-card shadow="hover" class="stat-card">
          <div class="stat-content">
            <div class="stat-icon icon-accent">
...
<style scoped>
.dashboard-stats-row { margin-bottom: 20px; }
.stat-card { height: 100%; cursor: default; transition: all 0.3s; background: var(--card); border-color: var(--border); color: var(--text); }
.stat-card:hover { transform: translateY(-4px); }
:deep(.stat-card .el-card__body) { padding: 16px; }
...
.stat-label { font-size: 13px; color: var(--muted); margin-bottom: 4px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.stat-value { font-size: 24px; font-weight: 600; color: var(--text); line-height: 1.2; margin-bottom: 2px; }
.stat-change { font-size: 12px; display: flex; align-items: center; gap: 2px; }
.stat-change-good { color: var(--success); }
.stat-change-bad { color: var(--danger); }
.stat-change-neutral { color: var(--muted); }
.stat-subtext { font-size: 12px; color: var(--muted); }
