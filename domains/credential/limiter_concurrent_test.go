...
func TestLimiterAcquireAll_Concurrent(t *testing.T) {
	...
	for i := 0; i < goroutines; i++ {
		...
		go func(idx int) {
			...
			release, err := l.AcquireAll(ctx, providerID, credentialID, identityHash, 0, 0, nil)
			if err != nil {
				failed.Add(1)
				return
			}
	...
	for p := 0; p < 3; p++ {
		for i := 0; i < 10; i++ {
			...
			go func(provider, idx int) {
				...
				release, err := l.AcquireAll(ctx, provider, 1, fmt.Sprintf("identity-%d", idx), 0, 0, nil)
				if err == nil {
					acquired.Add(1)
					time.Sleep(time.Microsecond)
					release()
...
func TestLimiterRelease(t *testing.T) {
	...
	release, err := l.AcquireAll(context.Background(), 1, 1, identityHash, 0, 0, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func BenchmarkLimiterAcquireRelease(b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		...
		for pb.Next() {
			...
			release, err := l.AcquireAll(ctx, i%10, i%100, fmt.Sprintf("identity-%d", i%50), 0, 0, nil)
			if err == nil {
				release()
			}
			i++
...
func TestLimiterStats_Concurrent(t *testing.T) {
	...
	release, err := l.AcquireAll(context.Background(), 1, 1, "test", 0, 0, nil)
	if err != nil {
		t.Fatal(err)
	}
