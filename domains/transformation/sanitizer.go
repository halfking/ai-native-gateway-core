...
// Sanitizer 能力感知的请求清洗 Transformer。
//
// 实现 transformation.Transformer 接口，可在 pipeline.Transform 阶段作为
// 一个独立 Hook 节点插入。catalogCode 来源（按优先级）：
//  1. 构造时的 catalogCode 字段（NewSanitizerWithCatalog）
//  2. ctx.Metadata["catalog_code"]（pipeline 上游传递）
//  3. 留空时跳过能力分支，仅做占位（不改 body）
type Sanitizer struct {
	catalogCode string
}

// NewSanitizer 构造默认 Sanitizer。
//
// catalogCode 在 Transform 时从 ctx.Metadata["catalog_code"] 读取。
// 当前生产入口 cmd/gateway-v2 与 domains/integration/full_pipeline.go
// 调用此构造器，期望 Sanitizer 自适应 metadata 中的 catalog。
func NewSanitizer() *Sanitizer {
	return &Sanitizer{}
}

// NewSanitizerWithCatalog 构造固定 catalogCode 的 Sanitizer。
func NewSanitizerWithCatalog(catalogCode string) *Sanitizer {
	return &Sanitizer{catalogCode: catalogCode}
}

// Name 返回 transformer 名称。
func (s *Sanitizer) Name() string { return "sanitizer" }

// Transform 应用能力感知的请求清洗。
//
// 行为：
//   - 读取 catalogCode（构造参数优先，其次 metadata）
//   - 调用 ApplyCapabilitySanitizer（nvidia_nim / deepseek / minimax 三分支）
//   - 写回 ctx.Request
//
// 失败安全：catalogCode 为空时跳过（不改 body），保持 pipeline 不阻断。
func (s *Sanitizer) Transform(ctx Context) error {
	if s == nil || len(ctx.Request) == 0 {
		return nil
	}
	code := s.catalogCode
	if code == "" && ctx.Metadata != nil {
		if v, ok := ctx.Metadata["catalog_code"].(string); ok {
			code = v
		}
	}
	if code == "" {
	...
	ctx.Request = ApplyCapabilitySanitizer(ctx.Request, code)
	return nil
}
