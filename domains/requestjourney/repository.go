...
func applyJourneyEvent(ctx context.Context, db interface {
	...
	rows, err := db.Query(ctx, `SELECT `+journeyEventColumns+`
		FROM request_state_transitions
		WHERE tenant_id=$1 AND request_id=$2 AND seq=$3 AND event_type IS NOT NULL`,
		event.TenantID, event.RequestID, event.Seq)
	if err != nil {
		return fmt.Errorf("load conflicting request journey event: %w", err)
	}
	defer rows.Close()
	existing, err := scanJourneyEvents(rows)
	if err != nil {
		return err
	}
	if len(existing) == 1 && sameJourneyEvent(existing[0], event) {
		return nil
	}
