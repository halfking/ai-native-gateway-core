...
func TestScriptSizes(t *testing.T) {
	...
	// last intentional script change (error-watermark fields, 11af45216).
		if s := sizes["record_request.lua"]; s != 12607 {
			t.Errorf("record_request.lua size = %d, want 12607 (update if script changed intentionally)", s)
		}
		if s := sizes["record_request_dual.lua"]; s != 11516 {
			t.Errorf("record_request_dual.lua size = %d, want 11516 (update if script changed intentionally)", s)
		}
}
