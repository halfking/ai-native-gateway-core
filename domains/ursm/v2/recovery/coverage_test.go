...
func TestValidateCoverageRejectsPendingMigration(t *testing.T) {
	m := New(newMini(t), "ursm:v2:")
	ctx := context.Background()
	...
	if err := m.rdb.Set(ctx, "ursm:v2:meta:coverage:pending", "migration", 0).Err(); err != nil {
		t.Fatalf("pending: %v", err)
	}
	if _, err := m.ValidateCoverage(ctx); err == nil {
		t.Fatal("pending coverage migration must block authoritative startup")
	}
}
