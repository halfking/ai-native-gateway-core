...
func init() {
	// Round 49 (2026-06-26): tolerate AlreadyRegisteredError so this
	// package can coexist with the legacy `compressor/` package (R1.6
	// migration). Both packages register the same metric names on
	// prometheus.DefaultRegisterer; without this safeguard, cmd/gateway-v2
	// tests panic with "duplicate metrics collector registration attempted"
	// because both init() run in the same process.
	//
	// Once R1.13 cutover deletes the old compressor/ package, this
	// helper can be replaced by plain prometheus.MustRegister again.
	...
	for _, c := range collectors {
		if err := prometheus.Register(c); err != nil {
			...
			if !errors.As(err, &are) {
				panic(err) // genuinely unexpected error
			}
