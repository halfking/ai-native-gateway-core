...
type Event struct {
	...
	// Hook-layer fields (AuditLogHook from domains/hooks/audit/hook.go)
	SessionID  string         `json:"session_id,omitempty"`  // Pipeline session identifier
	Stage      string         `json:"stage,omitempty"`       // "auth" / "routing" / "transform" / "response" / "post_response"
	Action     string         `json:"action,omitempty"`      // "allow" / "deny" / "modify"
	StatusCode int            `json:"status_code,omitempty"` // Upstream HTTP status
	Error      string         `json:"error,omitempty"`       // Error message (free-form)
	Metadata   map[string]any `json:"metadata,omitempty"`    // Cross-stage metadata
	CreatedAt  time.Time      `json:"created_at,omitempty"`  // Wall-clock timestamp the hook recorded the event
}
...
type Sink interface {
	...
	// Write/Close 用于 BatchWriter（domains/hooks/audit/batch_writer.go）
	// 单次事件 emit 默认走 Write([]*Event{event}) / Close 走 no-op。
...
// Write 适配 BatchWriter 批量调用：逐条调用 Emit。
func (s *LogSink) Write(events []*Event) error {
	for _, e := range events {
		if e == nil {
			continue
		}
		s.Emit(context.Background(), *e)
	}
	return nil
}

// Close LogSink 不需要释放资源。
...
// Write 适配 BatchWriter 批量调用：逐条调用 Emit。
func (m *MultiSink) Write(events []*Event) error {
	for _, e := range events {
		if e == nil {
			continue
		}
...
// Write 适配 BatchWriter 批量调用：逐条 Emit。
func (j *JSONSink) Write(events []*Event) error {
	for _, e := range events {
		if e == nil {
			continue
		}
		j.Emit(context.Background(), *e)
	}
	return nil
}

// Close JSONSink 不需要释放资源。
