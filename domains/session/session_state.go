...
redis.call('HSET', session_key,
  'current_credential_id', ARGV[2], 'current_cred_turns', '0',
  'current_cred_start_at', ARGV[3], 'current_cred_start_turn', total_turns,
  'current_model', ARGV[4], 'current_provider', ARGV[5])
redis.call('EXPIRE', session_key, ARGV[6])
redis.call('EXPIRE', rotations_key, ARGV[6])
return total_turns
`
	_, err := client.Eval(ctx, script,
...
func (sm *Manager) endCredRotation(ctx context.Context, sessionID string, data map[string]string) error {
	...
	// Reuse the session hash loaded by the caller to avoid a second read.
...
func (sm *Manager) StopSession(ctx context.Context, sessionID, reason string) error {
	...
	// Reuse the session hash loaded at the entry point for the snapshot.
...
func sessionFromRedisHash(sessionID string, data map[string]string) (*Session, error) {
	...
	return &Session{
		SessionID: sessionID, SessionKey: data["session_key"],
		APIKeyID: int(parseInt64(data[FieldAPIKeyID])), TenantID: data[FieldTenantID],
		TaskID: data["task_id"], Namespace: data["namespace"],
		CreatedAt: parseTimeOrZero(data["created_at"]), LastActive: parseTimeOrZero(data["last_active"]),
		ExpiresAt: parseTimeOrZero(data["expires_at"]), Status: data[FieldStatus],
		StoppedAt: parseTimeOrZero(data[FieldStoppedAt]), StopReason: data[FieldStopReason],
		RecoveredAt: parseTimeOrZero(data[FieldRecoveredAt]), ClientIP: data[FieldClientIP], ClientFP: data[FieldClientFP],
		CurrentCredentialID: int(parseInt64(data[FieldCurrentCredentialID])), CurrentModel: data[FieldCurrentModel],
		CurrentProvider: data[FieldCurrentProvider], TotalTurns: parseInt64(data[FieldTotalTurns]),
		FirstRequestAt: parseTimeOrZero(data[FieldFirstRequestAt]), LastRequestAt: parseTimeOrZero(data[FieldLastRequestAt]),
		TotalPromptTokens: parseInt64(data[FieldTotalPromptTokens]), TotalCompletionTokens: parseInt64(data[FieldTotalCompletionTokens]),
		TotalCostUSDCents: parseInt64(data[FieldTotalCostUSDCents]), CurrentCredTurns: parseInt64(data[FieldCurrentCredTurns]),
		CurrentCredStartAt: parseTimeOrZero(data[FieldCurrentCredStartAt]), CurrentCredStartTurn: parseInt64(data[FieldCurrentCredStartTurn]),
		Title: data[FieldTitle], Annotation: data[FieldAnnotation], Tags: data[FieldTags],
		FPSlotIndex: int(parseInt64(data[FieldFPSlotIndex])), FPSlotCredentialID: int(parseInt64(data[FieldFPSlotCredentialID])),
	}, nil
}
