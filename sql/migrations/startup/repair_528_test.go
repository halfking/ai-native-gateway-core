...
func TestMigration528RepairContract(t *testing.T) {
	up, err := os.ReadFile("528_repair_shared_pg_sticky_and_bodies_2026_07.sql")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, required := range []string{
		"HAVING count(*) > 1",
		"RAISE EXCEPTION '528: sticky_sessions contains",
		...
		if !strings.Contains(upSQL, required) {
			t.Errorf("migration 528 missing fail-closed contract %q", required)
		}
	}
	...
	if !strings.Contains(downSQL, "intentionally non-destructive") ||
		...
		t.Error("migration 528 down must preserve the forward-compatible schema")
	}
	if strings.Contains(upSQL, "DELETE FROM public.sticky_sessions") {
		t.Error("migration 528 must not delete duplicate sticky data")
	}
}
