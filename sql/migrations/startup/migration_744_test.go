...
// Migration 744 (252 SQL 日志审计第六轮, 2026-09-24) adds the partial indexes
...
//
// The contract pins the invariants that make 744 safe and effective:
//
...
//	    with psql --single-transaction; 744 ships through the Go ensure
...
func TestMigration744PartialIndexesContract(t *testing.T) {
	upBytes, err := os.ReadFile("744_sql_audit_partial_indexes.sql")
	if err != nil {
		t.Fatal(err)
	}
	up := string(upBytes)
	...
	if !strings.Contains(upCompact, outboxIdx) {
		t.Errorf("744 must create idx_session_aggregate_outbox_done_completed_at CONCURRENTLY with the status='done' predicate")
	}
	...
	for _, frag := range []string{"WHERE STATUS = 'DONE'", "COMPLETED_AT < NOW() - INTERVAL '7 DAYS'", "ORDER BY COMPLETED_AT"} {
		if !strings.Contains(reaperCompact, frag) {
			t.Errorf("reaper trimDoneRows no longer matches its 744 F-A predicate fragment %q — re-audit migration 744 A against the new trim SQL", frag)
		}
	}
	...
	for _, frag := range []string{
		...
		if !strings.Contains(upCompact, frag) {
			t.Errorf("744 digest section lost fragment %q", frag)
		}
	}
	...
	if !strings.Contains(backfillCompact, "WHERE T.DIGEST IS NULL") ||
		...
		t.Error("digest backfill SELECT no longer filters digest IS NULL / orders by (ts, id) — re-audit migration 744 B against the new scan shape")
	}
	...
	if strings.Contains(upCompact, "BEGIN;") || strings.Contains(upCompact, "COMMIT;") {
		t.Error("744 must NOT wrap its body in BEGIN/COMMIT — CREATE INDEX CONCURRENTLY cannot run inside a transaction block")
	}
	if got := strings.Count(upCompact, "CREATE INDEX CONCURRENTLY IF NOT EXISTS"); got != 3 {
		t.Errorf("744 must issue exactly 3 CONCURRENTLY builds (outbox + hot + per-partition template), found %d", got)
	}
	if got := strings.Count(upCompact, "CREATE INDEX IF NOT EXISTS"); got != 1 {
		t.Errorf("744 must issue exactly 1 non-concurrent build (the ON ONLY metadata shell), found %d", got)
	}

	// ── C4: down drops the named indexes + partition leftovers ─────────
	downBytes, err := os.ReadFile("744_sql_audit_partial_indexes.down.sql")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, idx := range []string{
		...
		if !strings.Contains(downCompact, idx) {
			t.Errorf("744 down missing %q", idx)
		}
	}
	if !strings.Contains(downCompact, "I.INHPARENT = 'PUBLIC.SESSION_TURNS'::REGCLASS") {
		t.Error("744 down must sweep unattached per-partition leftover indexes")
	}
}
