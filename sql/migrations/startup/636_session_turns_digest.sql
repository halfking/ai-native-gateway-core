-- Migration 636: persist nullable turn digest JSONB in parent and hot tables.
-- This migration is schema-only; runtime writers may begin populating digest later.
...
COMMENT ON FUNCTION public.promote_session_turns_hot_to_partition(INTERVAL, INTEGER) IS
    'Atomically moves one validated cold batch from session_turns_hot into attached monthly partitions using DELETE RETURNING plus INSERT, retaining nullable digest JSONB.';

DO $$
DECLARE
    v_parent_columns INTEGER;
    v_hot_columns INTEGER;
BEGIN
    SELECT count(*) INTO v_parent_columns
    FROM pg_attribute
    WHERE attrelid = 'public.session_turns'::regclass
      AND attnum > 0 AND NOT attisdropped;
    SELECT count(*) INTO v_hot_columns
    FROM pg_attribute
    WHERE attrelid = 'public.session_turns_hot'::regclass
      AND attnum > 0 AND NOT attisdropped;

    IF v_parent_columns <> 51 OR v_hot_columns <> 51 THEN
        RAISE EXCEPTION 'Migration 636 expected 51 parent/hot columns, found parent=% hot=%',
            v_parent_columns, v_hot_columns;
    END IF;
    ...
    IF EXISTS (
        ...
        SELECT 1 FROM parent_columns p FULL JOIN hot_columns h USING (attname)
        WHERE p.attname IS NULL OR h.attname IS NULL
           OR p.atttypid <> h.atttypid OR p.atttypmod <> h.atttypmod
           OR p.attnotnull <> h.attnotnull
    ) THEN
        RAISE EXCEPTION 'Migration 636 parent/hot column parity failed';
    END IF;
    ...
    IF EXISTS (
        ...
        WHERE attrelid IN ('public.session_turns'::regclass, 'public.session_turns_hot'::regclass)
          AND attname = 'digest'
          AND (atttypid <> 'jsonb'::regtype OR attnotnull)
    ) THEN
        RAISE EXCEPTION 'Migration 636 digest must be nullable JSONB on parent and hot';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'Migration 636 session_turns_with_current_month must use security_invoker=true';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'Migration 636 digest is missing from the current-month view';
    END IF;
END $$;
