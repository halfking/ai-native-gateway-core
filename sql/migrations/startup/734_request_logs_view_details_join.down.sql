-- ===========================================================================
-- File:          sql/migrations/startup/734_request_logs_view_details_join.down.sql
-- Migration:     734 down
-- Purpose:       重建 710 形态视图（NULL 占位、无 details JOIN）。
--                体与 710 主块逐字一致；不重写 710 的 ledger 行。
-- 依赖：733 仍存在（本 down 不动表族）。
-- ===========================================================================
...
BEGIN
  ...
  -- 本迁移的 734 details-joined 体同样含 session_turns，若只探前者，
  -- 734 体被误判为 710 体而跳过重建，后续 733 down DROP 表族时
