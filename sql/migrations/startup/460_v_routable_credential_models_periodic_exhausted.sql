-- Migration 460: Add periodic_exhausted to v_routable_credential_models unavailable_reason
--
-- Date: 2026-07-27
--
-- Root cause:
--   The unavailable_reason CASE in v_routable_credential_models did not handle
--   quota_state = 'periodic_exhausted'. Credentials in this state had NULL
--   unavailable_reason, causing routingBlockReason() to return NULL and allowing
--   traffic to be routed to exhausted credentials.
--
-- Fix:
--   Add 'periodic_exhausted' to the quota_state CASE, matching the existing
--   pattern for 'permanently_exhausted' and 'balance_exhausted'.
--
-- Idempotent: YES — CREATE OR REPLACE VIEW
-- Downstream: No dependency on 459 (unrelated table columns)
...
COMMENT ON VIEW public.v_routable_credential_models IS
    'Routable credential-model bindings with quota/availability state gates. '
    'Sync with sql/objects/views/v_routable_credential_models.sql. '
    'Updated by migration 460 (2026-07-27) to add periodic_exhausted to quota_state check.';
