-- ===========================================================================
-- File:          sql/migrations/startup/732_request_logs_view_details_join.sql
-- Migration:     732
-- Database:      llm_gateway
-- Purpose:       会话存储解耦 v3（docs/storage/2026-09-20-session-storage-
--                decoupling-plan.md §3.2）：710 拼装视图的 session 分支
--                LEFT JOIN session_turn_details(_hot)（731 特征层），把 30 个
...
--                （无源或废弃，登记于 710 列映射契约）。
--
-- 依赖：731（session_turn_details 表族）。
-- Down: 732_request_logs_view_details_join.down.sql 重建 710 形态。
-- ===========================================================================
...
BEGIN
  ...
  --    732 变更点仅 30 个 d.* 替换；其余与 710 逐字一致。
  proj := $proj$
NULL::bigint AS id
...
$proj$;
  ...
  COMMENT ON VIEW public.request_logs_with_current_month IS
    '会话存储解耦 v3（732）: 710 拼装体升级——session 分支 LEFT JOIN '
    'session_turn_details(_hot)（731 特征层，键 tenant_id+request_id+partition_date），'
...
VALUES ('732', 'session storage v3: canonical view session branches LEFT JOIN session_turn_details(_hot), 30 NULL placeholders replaced by real feature columns')
