...
func TestMigration620ProviderErrorAggregatorState(t *testing.T) {
	migration, err := os.ReadFile("620_provider_error_aggregator_state.sql")
	if err != nil {
		t.Fatal(err)
	}
	...
	down, err := os.ReadFile("620_provider_error_aggregator_state.down.sql")
	if err != nil {
		t.Fatal(err)
	}
