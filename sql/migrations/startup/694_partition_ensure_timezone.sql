-- Migration 694: 固定分区 ensure 函数的会话时区
--
-- timestamptz 的 date_trunc/now() 会读取会话 TimeZone。生产分区边界
-- 约定为 Asia/Shanghai；若由 UTC 会话调用，月初边界可能偏移 8 小时，
-- 重现 473 类 default 缝隙。所有初始化表达式均在 SET LOCAL 之后执行。
--
