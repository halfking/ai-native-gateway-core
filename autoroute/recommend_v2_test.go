...
func TestRecommendV2_AvailabilityGate(t *testing.T) {
	...
	results := idx.RecommendV2(context.Background(), TaskCode, ClassificationSignals{}, ProfileSmart, "", 3)
...
func TestRecommendV2_CorrectionScoreApplied(t *testing.T) {
	...
	results := idx.RecommendV2(context.Background(), TaskCode, ClassificationSignals{}, ProfileSmart, "sess-1", 3)
	if len(results) < 2 {
...
func TestRecommendV2_DisabledHotPoolUsesAllCandidates(t *testing.T) {
	...
	results := idx.RecommendV2(context.Background(), TaskChat, ClassificationSignals{}, ProfileSmart, "", 3)
	if len(results) == 0 {
...
// TestRecommendV2_LegacyScoringBranch verifies NET-013: when both
// UseChannelQualityRouting and UseSimplifiedScoring are disabled, RecommendV2
...
func TestRecommendV2_LegacyScoringBranch(t *testing.T) {
	...
	candidates := []Candidate{
		...
		{CredentialID: 2, CanonicalID: 2, CanonicalName: "cheap", UnavailableReason: "", Tags: []string{"chat"}, SuccessRate: 0.95, P95LatencyMs: 2000, UnitPriceInPer1M: 1, UnitPriceOutPer1M: 1},
	}

	idx := &Index{entries: candidates, lastRefresh: time.Now()}
	results := idx.RecommendV2(context.Background(), TaskChat, ClassificationSignals{EstimatedTokens: 1000}, ProfileSpeedFirst, "", 2)
	if len(results) == 0 {
		t.Fatal("expected results")
	}
	if results[0].Candidate.CanonicalName != "speedy" {
		t.Fatalf("expected legacy speed-first scoring to prefer speedy, got %s", results[0].Candidate.CanonicalName)
	}
}
