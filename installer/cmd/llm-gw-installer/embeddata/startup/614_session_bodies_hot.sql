...
-- Enable row level security so the canonical reader enforces tenant isolation.
ALTER TABLE public.session_bodies_hot ENABLE ROW LEVEL SECURITY;
...
-- Add canonical unique constraint required by the Session V2 migration contract:
-- (tenant_id, request_id, partition_date) — the contract-mandated
-- session_bodies_with_current_month identity.
ALTER TABLE public.session_bodies_hot
...
-- Explicit column list (never wildcard) matches the session_bodies contract.
