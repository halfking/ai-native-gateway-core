-- Fresh-installer bootstrap: final Session V2 hot-path objects.
-- This installer-only asset projects the final 526 + 636 state onto the
-- current baseline. It is not an upgrade migration and must not be applied
...
BEGIN
    ...
    IF EXISTS (
        SELECT 1
        FROM (VALUES ('session_turns'), ('session_turns_hot')) expected(table_name)
        WHERE NOT EXISTS (
            SELECT 1
            FROM information_schema.columns c
            WHERE c.table_schema = 'public'
              AND c.table_name = expected.table_name
              AND c.column_name = 'digest'
              AND c.data_type = 'jsonb'
              AND c.is_nullable = 'YES'
        )
    ) THEN
        RAISE EXCEPTION 'installer session-turns bootstrap requires nullable JSONB digest on parent and hot tables';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'installer session-turns bootstrap view is missing digest';
    END IF;
END $$;
