...
func TestComputeConsensus_Skipped(t *testing.T) {
	// Plain skipped (manual_disabled etc.) must never change counters or state.
...
func TestComputeConsensus_Skipped_EndpointIDRequired_ResetsFailures(t *testing.T) {
	// A skipped probe with errCode="endpoint_id_required" on a broken_confirmed
	// binding (previously mis-probed with raw_model_name → 404) must clear the
	// failure counters and set state back to 'recovering' so the binding
	// re-enters the probe queue once outbound_model_name is configured.
	sc, applied, succ, fail, st := computeConsensusForTest("skipped", "broken_confirmed", "endpoint_id_required", 0, 3)
	if sc != "unchanged" || !applied || succ != 0 || fail != 0 || st != "recovering" {
		t.Errorf("endpoint_id_required skipped on broken: got (%s,%v,%d,%d,%s) want (unchanged,true,0,0,recovering)",
			sc, applied, succ, fail, st)
	}
}

func TestComputeConsensus_Skipped_EndpointIDRequired_RecoveringState(t *testing.T) {
	// Same fix applies when state is 'recovering' (not yet broken_confirmed).
	sc, applied, succ, fail, st := computeConsensusForTest("skipped", "recovering", "endpoint_id_required", 0, 1)
	if sc != "unchanged" || !applied || succ != 0 || fail != 0 || st != "recovering" {
		t.Errorf("endpoint_id_required skipped on recovering: got (%s,%v,%d,%d,%s) want (unchanged,true,0,0,recovering)",
			sc, applied, succ, fail, st)
	}
}
...
func TestComputeConsensus_AuthFailureCountsAsFailure(t *testing.T) {
	...
	_, _, succ, fail, st := computeConsensusForTest("auth", "unknown", "decrypt_error", 0, 1)
...
func computeConsensusForTest(status, prevState, errCode string, prevSucc, prevFail int) (
	...
	r := &ModelProbeRunner{}
	return r.computeConsensus(status, prevState, errCode, prevSucc, prevFail)
}
