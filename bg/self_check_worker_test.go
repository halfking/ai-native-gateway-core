...
func TestSystemAPIKeyHashMatches(t *testing.T) {
	const secret = "test-secret-key"
	const rawKey = "sk-selfcheck-deadbeefcafef00d"
	hash := authentication.HashAPIKey(secret, rawKey)

	if !systemAPIKeyHashMatches(secret, rawKey, hash) {
		t.Fatal("expected matching system key hash to be accepted")
	}
	if systemAPIKeyHashMatches(secret, rawKey, authentication.HashAPIKey("other-secret", rawKey)) {
		t.Fatal("expected hash generated by another gateway secret to be rejected")
	}
}
