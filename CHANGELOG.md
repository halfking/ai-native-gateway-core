...
### Fixed
- **Streaming P0 second audit follow-up (2026-08-28)**: error-body draining now continues after read errors; successful buffered attempts commit trailing partial SSE data before reporting success; serialized capture records only complete successful writes; checkpoint hooks run once per monotonic state. Regression coverage and audit evidence are in `docs/audit/2026-08-28-streaming-p0-second-followup-audit.md`.
...
- **24h deep-audit P1 follow-up (2026-08-28, audit-24h-20260828-r3)**: anthropic stream path now detects empty response on `message_stop` and emits `KindEmptyResponse` to fail over to next candidate (parity with non-stream detector at executor_anthropic.go:1273). Added `chunkCount` tracking in `domains/transformation/anthropic/anthropic_passthrough_stream.go` (counts only content-bearing `content_block_*` events, not envelope `message_start/stop/delta`), EOF-time empty check (`chunkCount==0 && (capture==nil || no tokens)`), `StreamOutcome.Kind` field (`errorsx.ErrorKind`), and 4 regression tests (empty SSE fixture + usage-only empty + non-empty positive + nil-capture). Q3 path (AnthropicToOpenAIStream / AnthropicToResponsesStream) noted as TODO for future parity — those translators run independent SSE loops and don't invoke this detector yet. When upstream sends `message_start` + `message_stop` with zero content blocks, executor now routes to the next candidate via `streamInterruptedError{kind: KindEmptyResponse, resumable: true}` instead of recording a successful empty stream.
- **24h deep-audit P1 follow-up (2026-08-28, audit-24h-20260828-r4)**: extend the r3 empty-response fail-over to the Q3 (`AnthropicToOpenAIStream`) and Phase E (`StreamAnthropicSSEToResponses`) translators. Both run IR-based SSE loops that emit a non-zero chunk count for envelope events (role prelude, response.created) so the r3 `chunkCount==0` rule would misfire; introduced a shared `anthropic.IsAnthropicStreamEmpty(emittedContent, inputTokens, outputTokens)` helper in `stream_support.go` plus an `emittedContent` boolean tracked inside each translator's `writeChunk*` closure (set true for non-empty `Delta.Content`, `Delta.ReasoningContent`, or non-empty tool-call deltas). The two clean-EOF paths in each translator now check `IsAnthropicStreamEmpty` and return `KindEmptyResponse` with `Resumable=true` on empty. Added 4 Q3 regression tests + 2 Phase E regression tests covering empty fixture, usage-only empty, non-empty positive, and nil-capture (Q3). Closes the TODO flagged in `anthropic_passthrough_stream.go:34-40`.
- **24h deep-audit P2 follow-up (2026-08-28, audit-24h-20260828-r4)**: continue the r3 / P1-14 `SafeHGetAll` migration from the credential / dispatch / requestjourney sites to other domains that read hash keys. Migrated 5 raw `client.HGetAll(...).Result()` call sites:
  - `domains/session/v2/cache_v2_redis.go` (V2 governance cache Get): now uses `SafeHGetAll` with explicit `ErrKeyNotFound` → cache-miss handling (the original code already had `redis.Nil` handling, preserved).
  - `domains/session/preprocess/redis_store.go` (artifact store readManifest): `SafeHGetAll` + `ErrKeyNotFound` → new-manifest sentinel.
...
- **24h deep-audit P3 doc fixes (2026-08-28, audit-24h-20260828-r4)**: close the two remaining P3 items from the 24h audit handoff (docs-only changes; no behavior change).
  - `domains/streaming/attempt_commit_gate.go`: document why `Discard` acquires only `g.mu` (not `writeMu`). The asymmetry is intentional — Discard does no wire write, only resets in-memory state; the `g.discarded` flag is read under `g.mu` inside `WriteFrame`, so a concurrent WriteFrame either completes before Discard or sees `ErrAttemptDiscarded` without any interleaving that could emit frames after a Discard. Future contributors must not 'fix' the asymmetry by adding `writeMu` — it would deadlock any goroutine in `checkpointStateAdvanceUnderWriteLock` and is not required for correctness.
  - `cmd/gateway/main.go`: in the online-mode `else` branch, distinguish the three license-file states at startup so the operator can see whether the offline file was present + verified (informational, ignored in online mode), present + failed (already handled by the Warn branch), or absent (common case for fresh installs). Previously the same `"license verification successful"` log printed regardless of which state held, which made it hard to tell whether the license came from the DB / license API (authoritative) or from the offline file (which is OPTIONAL in online mode and ignored for authorization decisions).
...
- **热更新 prompt 预算配置（2026-08-24）**：新增系统配置 `gateway.max_prompt_tokens`，默认上限调整为 1048576（1M tokens），通过 `/api/admin/settings/gateway.max_prompt_tokens` 修改后约 5 秒内生效；保留 `LLM_GATEWAY_MAX_PROMPT_TOKENS` 作为无 settings registry 时的环境变量回退。
