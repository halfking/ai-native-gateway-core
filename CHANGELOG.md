...
### Fixed
- **探测 P0-2：网络类短梯长尾化 + request_failure pair 级频控 + 队列代际 attempt 重置 + root-cause 分类器 defer 断链根修（2026-09-26，r0926）**：①网络/瞬时类失败退避原短梯 4 步封顶 60s 永续（2026-09-25 实测 45 对滞留 60s 档、last_err_code 全为 connection_error/503/timeout/500，periodic 19.1 万次/48h 的主体来源）——`networkProbeBackoff()` 在 attempt 1..4 保留 5s/15s/30s/60s 后沿通用梯尾档 5m→1h→2h→6h 封顶（开关 `probe.network_chain_long_tail`，默认 true，false 回滚旧行为）；短暂抖动（<1h）发现速度不变，持续宕机对衰减到 6h 节奏（明示取舍，方案 §6/§7）。②request_failure 触发入队前查 `node_probe_state`：行带错误证据且探测已排程（next_retry_at 未到）或距上次探测不足 `probe.request_failure_min_gap_seconds`（默认 60，0=旧行为）则跳过本次触发——同分钟第 N 条业务失败不再引发第 N 次探测；健康停放行（INV-2）永不命中，periodic 泵不受频控；真 SQL 语义在一次性库（TEST_PG_URL 铁律）钉桩。③队列任务代际 attempt 重置（r0925 遗留 1）：`ProbeService.Run` 的 attempt 改为 max(task.Attempt, state.consecutive_failures+1)（与 legacy runOne 同算法），泵再生成的任务不再把梯位塌回 5s 档、protocol 族不再每代两次快探。④**root-cause 分类器 defer 断链（本轮新发现 P1）**：probeDirect/probeGateway 匿名返回值 + defer 改局部变量——`return r` 拷贝先于 defer，分类/标注永远到不了调用方（2251/2252 实测 `llmgw_node_probe_root_cause_total` protocol/gateway 占比恒 0、endpoint_build 被记 node、日志全打 "node/upstream fault"）；改命名返回值 `(r nodeProbeRoundResult)` 根修，`probeBackoffForDirectOutcome` 的 protocol 6h 停放自此在 direct 路径真正生效。⑤存量清洗：`last_direct_ok=TRUE AND last_err_code<>''` 7 行僵尸（凭据 36/43/47 全部 manual_disabled、其中 4 行无绑定，"下次相遇收敛"预测不成立）一次性清错误码归 healthy-parked 形态，复检 0 行。部署验证：P0-1+P0-3（2252=19eb3153）上线后 `gw_rpm_exceeded` 探测弹回 738/h→0（仅启动 keyInfo 缓存窗 290 条）、node_probe_runs ~5,000/h→~1,610/h；P0-2 效果验收（方案 §8.3 SQL ④≤20,000/48h、⑤短梯三档 ≤5 对）待 48h 窗口。测试：`go test ./... -count=1` 全绿（新增 6 测试函数含真库谓词钉桩；3 处旧钉桩随长尾契约演进）。审计文档：`docs/handoff/20260926-probe-p02-longtail-throttle-audit.md`。
- **聊天桥 empty-stream gate flush 漏提 finish_reason（2026-09-24，r0924 真机注入实锤）**：`StreamChatWithPendingCapture` 的 empty-stream gate（用于 NIM/MiniMax-style relay 第一包不带 content 的场景）在 flush buffered chunks 给客户端时只递增 chunkCount，没有把 chunk.FinishReason 上提到外层 finalFinishReason。EOF 时 finalFinishReason 仍为空，本应进入 `chat-bridge d8a7849fd` 良性完成分支（条件 `finalFinishReason != "" && chunkCount > 0`，合成 `data: [DONE]\n\n` 后归类为成功），却落到 §11.6 严格失败分支并向客户端发 `upstream_incomplete` 信封——下游 OpenAI Responses SDK 在收到这个 SSE 帧后报 "supplier unstable / provider unavailable"。`responses_bridge.go:1187`（R59 S1-F2）与 `anthropic_stream.go:854`（良 EOF 语义独立）此前已具备等价处理，此次把 chat 桥与之对齐。工单原文 `/v1/responses` stops after one request + supplier unstable，复现失败点实际在 chat-bridge（`/v1/responses` 与 `/v1/chat/completions` 在 minimax 上都走 chat completions 上游）。修复：`domains/streaming/stream.go` 空流门 flush 循环里加 `finalFinishReason` 提升，与 main-loop 路径同语义（parseError 容忍）；行为对称不引入新分支条件。回归钉桩 `domains/streaming/stream_eof_test.go::TestStreamChatWithPendingCapture_BenignEOFAfterFinishReason_MiniMaxProductionShape` 用请求 `c96df1a8a50154667e8f1d40fa64b2bd`（2026-09-23 17:21）原始 wire shape（role-only → finish_reason:"tool_calls"+tool_calls → usage-only → 无 [DONE]）三 chunk 序列钉桩，无修复必红。测试：`go test ./domains/streaming/ -count=1` 全绿（含现有 `TestStreamChatWithPendingCapture_BenignEOFAfterFinishReason` 旧 2-chunk 形态）；live 8782（build 2242，ef87317c+uncommitted stream.go）原失败 posture（chat/completions stream + tools, finish_reason:"tool_calls" 收尾）回归 audit `success:true stream_chunks:9`，`upstream EOF without [DONE] after finish_reason — benign non-compliant close (§11.6 parity)` 日志确认良性路径生效；`/v1/responses` non-stream + stream + stream+tools 三姿势全部正常。GitHub main `15eb0f834 fix(streaming): lift finish_reason through empty-stream gate flush`（+96 行 / 2 文件，未触动其他 pre-existing 工作区改动）。现场容器 version.json 当前 `git_sha=ef87317c`，是历史"在镜像 build 后再做 fix 并 docker cp binary"的偏差，代码已生效，下一轮正式 rebuild 会刷新此字段；本轮暂不强行 patch live gateway 的 version.json 维持与 build 输入一致。

### Added
- **R62 installer 审计 P2/P3 收口 (2026-09-24)**：`fix(installer): R62 安装模式+自动激活审计轮 P2/P3 收口 + 审计取证` 落地。补一轮二级修复 `R62 audit round 2 - offline installDir parameterization + activate flag rename`（installer 侧，offline 部署 installDir 参数化 + activate flag 重命名）+ `fix(license-authority): R62 audit round 2 - panic to error in newTrialLicenseKey`（trial license 失败从 panic 改 error 返回）。版本随动 `chore(version): r0924 R62 后 build 2244`；`deploy(audit): R62 例行部署轮` 完成 154/245 双侧落地。`docs(readme): 同步 installer 部署模式到 8 语言 README` 文档随动。审计取证：[`docs/audit/2026-09-24-install-mode-activation.md`](docs/audit/2026-09-24-install-mode-activation.md)；部署轮：[`docs/handoff/2026-09-24-r62-deploy-round-handoff.md`](docs/handoff/2026-09-24-r62-deploy-round-handoff.md)。代码 commit 链（均已 push 至 origin/main，可 `git show <sha>` 验证）：`3455a7a82` → `76011ac6e` → `dad3377d1` → `3957c1423` → `97ebdc3cc` → `3b4ed5df2`。**注**：本条描述由 R66 入口 ② 批判式审计轮重写，前版含三处伪造 SHA（`76b7ed029`/`78aa2577e`/`153ad093a`，均 `git cat-file -t` 报 `Not a valid object name` 或误标为 R62 实为 R63 48h 审计轮的 `153ad093a`）与一处伪造路径（`docs/audit/2026-09-24-r62-install-mode-activation.md`，实为 `-install-mode-activation.md`），均由子代理虚构，R66 ② 全部实锤并替换为可验证的真实 commit。
...
### Fixed
- **§11.6 survival post-DONE 第二终端泄漏（2026-09-23，R58 真机注入实锤）**：245 真机故障注入（单候选 committed-then-EOF）实证 survival Terminal seam 渲染终态 envelope+[DONE] 后，chat 共享 post-execute 错误面 prewarm 分支再写一帧 model_not_found——绕过 R56 P2-1 终端闩锁（该闩锁仅 survival_coordinator.go:1116 消费）。修复：runSurvivalCoordinator 终态哨兵 `errSurvivalTerminalRendered`（多 %w 保 errorsx As-链）+ handler 错误面 `blackholeResponseWriter`（bookkeeping 全保留、线上写静默）；钉桩 `survival_committed_broken_e2e_test.go` 真红验证（无修复时逐字节复现泄漏形态）。messages/responses 既有 usedSurvival 早退不受影响。同轮 Handoff-B 收口：chunk_buffer（s8_burst 场景实为 stream:false、最新实跑 871 req/s 否证 ~600 封顶、idle-flush 无定时器）与 error_detector_ring/ring_error_counter（换型点 WeightedRouter 生产零持有、内存主张失真 ~25×）三验证后删除。审计文档：`docs/audit/2026-09-23-r58-48h-audit-round.md`。
- **auto route rollup 42P10 全失败修复（2026-09-18, 迁移 726）**：154 生产（build 2138，09-18 07:35 部署后）`auto route listener: refresh failed error="rollup credential_model_index: insert: … no unique or exclusion constraint matching the ON CONFLICT specification (SQLSTATE 42P10)"` 每 5 分钟 + 每次 credentials/bindings NOTIFY 必现。根因：`(bucket, credential_id, raw_model)` 三列历史上叠了三套等价唯一索引（347 `credential_model_index_hot_unique_key` / 354 `idx_credential_model_index_hot_unique` / 718 所述第三套），718 冗余清理按"三留一"drop 掉前两者，但其在 154/245 共享 PG17 上并不存在（354 的 IF NOT EXISTS 未在该库生效，账本 version 混有 'V359' 非数字值），导致 hot 表唯一索引归零、rollup INSERT 必然 42P10；8h retention 的 promote 随后把 hot 存量搬空（实测 0 行），`credential_model_index_with_current_month` 最新桶冻结在 09-17 14:45，auto 路靠过期快照决策（refreshIndexSQL 无新鲜度下限，服务未断但指标停止进化——本日"kaixuan/auto 是否生效"审计的根因）。修复：`sql/migrations/startup/726_restore_credential_model_index_hot_unique(.down).sql` 幂等去重（ctid 守卫）+ `CREATE UNIQUE INDEX IF NOT EXISTS idx_credential_model_index_hot_unique`（718 选定保留名；新装库 354 已建则 no-op）；`model_task_index` 唯一约束健在无需修。契约测试 `migration_726_test.go` 钉四不变量：索引元组与 `bg/auto_index_refresher.go` ON CONFLICT 元组绑定解析（防拷贝漂移）、去重守卫先于建索引、任何 startup 迁移不得 drop 保留侧索引（718 类错误回归门）、installer embeddata 字节级同步。测试：`go test ./sql/migrations/startup/ -count=1` 全绿（干净 worktree@HEAD 验证）。schema 快照（`sql/schema`、`deploy/sql/schemas/baseline`、objects）为 dump 生成物未手改，下次 `sql/scripts/dump-schema.sh` 自然收敛。审计文档：`docs/changelogs/2026-09-18-autoroute-rollup-42p10.md`。遗留：① 本地 Windows bg 包构建受 `storage_retention_worker` syscall.Statfs 平台限制（既有，Linux 目标不受影响）；② 共享库 `schema_migrations.version` 存在 'V359' 等非数字脏值；③ 154 运行时 env 的 DB 口令曾在诊断输出中暴露，建议轮换；④ 718 的"三留一"核实仅覆盖审计本地库，共享库漂移无前置探针（726 契约测试为事后门）。
...
- **PostgreSQL Instance Sync Tool (2026-09-08)**: 新增实例 inventory、确定性 plan、结构影响矩阵、校验备份、缺库 bootstrap 和 insert-only 数据合并。
  ...
  - **CLI**: 统一入口分发 `apply-schema`/`apply-data`/`verify`/`restore`；`all` 仍 fail-closed
  - **Verify/restore**: FK 孤儿与禁用触发器校验；回滚只允许恢复到新库名
  - **Experience**: `docs/experience/2026-09-08-pg-instance-sync.md` 与 `docs/skills/pg-instance-sync-252/SKILL.md`
...
- **184 服务器引用全面清理（rule 39 铁律 1 + rule 47）**：
  ...
  - 关键迁移：`CONFIGURATION_GUIDE.md` `.env.184.enc`→`.env.154.enc`、`.env.71.enc`→`.env.252.enc`、`__PUB_IP_1__` `<env:HOST_184_IP>`→`<env:HOST_154>`；`CONFIGURATION_GUIDE.md` 示例 IP `<env:HOST_184_IP>`→`<env:HOST_154>`；`compression-bench/README.md` K8s DB 端口转发 184→252、端口 18432→25232；`verify-model-fetch/main.go` 注释 `71/184`→`154/252`。
...
- **PROJECT_CONFIG.md 敏感信息脱敏（rule 31 §1 + rule 39）**：
  - SSH 地址 `<env:HOST_184_IP>:25022` → `<env:HOST_154>:25022`（184 已废弃，rule 31 §1）
  ...
  - SSH 命令示例：`ssh admin@<env:HOST_184_IP>` + `sudo su -` + 明文密码 → 证书认证 `ssh -i <env:SSH_KEY_154> root@<env:HOST_154>` + sshpass fallback
...
- **PROJECT_CONFIG.md credential redact**（rule 31 §1 + rule 39）：
  - SSH 地址 `<env:HOST_184_IP>:25022`（已废弃）→ `<env:HOST_154>:25022`（rule 31 §1：184 废弃）
  - 默认用户 `admin` → `root`（匹配 154 SSOT metadata.yaml）
  ...
  - SSH 命令示例：`ssh admin@<env:HOST_184_IP>` + `sudo su -` + 明文 root 密码 → 证书认证
...
- 删除根目录含硬编码凭据的调试脚本（均已进入 git 历史，密钥需线下轮换）：
  ...
  （volcengine ark API key）、`.tmp_check154.py`、`check_154_routing.sh`（root@<env:HOST_154_ALT_IP> SSH 探测）。
...
- **Outbox metrics wiring + Grafana alerts/dashboard (2026-08-12)**: `internal/outbox/dispatcher.go`、`internal/outbox/dispatcher_test.go`、`deploy/monitoring/grafana-alerts/outbox.yaml`、`deploy/grafana/dashboards/outbox-delivery.json` — `metrics.go` (commit `991acc5e3`) 声明了 6 个 Prometheus 指标但 `dispatchOne` 从未调用，导致 `/metrics` 端点上 sent/failed/retried/duration 永远是 0。修复：`dispatchOne` 三个出口路径（success/http-fail/unmarshal-fail）全部接入 `RecordEventSent` / `RecordEventFailed` / `RecordEventRetried` / `ObserveDeliveryDuration`；新增 6 条 PromQL 告警（DLQ 累积/失败率 >10%/pending >1000/P99 >5s/HMAC 失败/Provider-Unknown >5%）；新增 Grafana 仪表盘 6 个 panel；6 个 sqlmock 单元测试覆盖每个分支。详见 `docs/changelogs/2026-08-12-outbox-metrics-wiring.md`。245 部署验证（seq=1491）：`/metrics` 端点确认 3 个 counter/gauge 注册，unit test 证明 wiring 在每个出口路径上 fire。
...
- **自检队列 SSE 二轮审计修正 (2026-08-12 second pass)** — 对一轮修复 commit `40f2880f` 的复审，发现并修复 3 个遗漏。详见 `AUDIT_PROBE_STREAM_LIFECYCLE_20260812.md` §Second-pass findings:
  - **[P1] daily selfcheck 生命周期分裂**: `bg/credential_selfcheck.go` `publishSelfcheck` 每次调用生成新 ns-timestamp ID，`runOne` 的 in-flight 与终态调用产生两个不同 ID 的 tile（一轮修了 node_probe/integrity 但漏了 selfcheck）。修复：`publishSelfcheck` 接收 `runID`（self_check_runs DB 行 ID），ID 格式 `selfcheck:<cred>:<runID>`，两处调用共用；no-routable 快速失败路径补发终态 fail 事件。新增 `TestCredentialSelfcheck_TaskID_StableAcrossLifecycle`。
  - **[P1] RecordWithOrigin read-modify-write 竞争**: `admin/probe_stream_redis_store.go` 一轮修复把 prev-status `HGet` 放在 pipeline 外，与并发 transition 竞争（两个 transition 都读到旧 prev → 都 ZREM 错的 key → HSet 互相覆盖），且多一次 Redis 往返。修复：整个 lane transition（HGet prev → ZREM → ZADD → HSET → trim）移入单 Lua 脚本 `recordTransitionScript` 原子执行；detail SET + PUBLISH 留在独立 best-effort pipeline。新增 `TestRecordTransitionScript_Loaded` 断言脚本注册与关键不变量。
  - **[P3] 死代码清理**: 删除无生产调用方的 `ProbeRedisStore.Record` legacy 包装与 `trimProbeQueue`（已内联进 Lua）。
...
- **245 预生产磁盘清理 + stderr 日志轮转 (2026-08-04)**:
  - 修复 245 (<env:HOST_245_IP>) 预生产服务器 `/dev/vda3` 磁盘撑爆（40G 已用 36G/97%，仅 1.5G 可用）
...
- Re-pulled `pg_dump --schema-only` from `pg-252-pg17` (<env:HOST_252_INTERNAL_IP>)
...
- **DB schema synced from 252.** The local `llm_gateway` DB was
  rebuilt from `pg_dump --schema-only` of `pg-252-pg17` (<env:HOST_252_INTERNAL_IP>),
...
- Live ops changes (NOT in this repo — on 252 / 154):
  ...
  - 252: new file `conf.d/files-kxpms-cn-9444.conf` — 9444 ssl proxy_protocol
    ...
    proxy_pass to `<env:HOST_154_INTERNAL_IP>:5212` (Cloudreve on 154).
