...
type EndpointLite struct {
	...
	Weight       int     // tiebreaker for two endpoints with identical match
...
//   - Ties on MatchRule are broken by EndpointLite.Weight ascending (lower
//     weight = higher priority, matching the historical weight semantics
//     in provider.Candidate.Weight).
...
func Select(cand CandidateLite, wantProtocol Protocol, wantFamily string) Decision {
	...
	// Sort endpoints by (isPrimary desc, weight asc) to keep ordering
	// deterministic across runs; tie-breaking only affects log emission
	// because Stage 1A-1C share the same endpoint row when only one
	// matches.
...
func pickBest(eps []EndpointLite, wantProtocol Protocol, wantFamily string) *EndpointLite {
	...
	// Sort: primary first, then by weight ascending (lower weight =
	// higher priority, matching provider.Candidate.Weight semantics).
	sort.Slice(matched, func(i, j int) bool {
		if matched[i].IsPrimary != matched[j].IsPrimary {
			return matched[i].IsPrimary
		}
...
// across re-routes) and break ties by lower weight ascending.
func pickFamilyHit(eps []EndpointLite, wantFamily string, wantProtocol Protocol) *familyHit {
	...
	for v, list := range byFamily {
		...
		cands = append(cands, cand{vendor: v, eps: list, count: len(list), minW: minW})
	}
	sort.Slice(cands, func(i, j int) bool {
		if cands[i].count != cands[j].count {
			return cands[i].count > cands[j].count
		}
		if cands[i].minW != cands[j].minW {
			return cands[i].minW < cands[j].minW
		}
