...
// durableTaskSchemaSQL mirrors sql/migrations/startup/515_durable_llm_tasks.sql.
// Keep this startup path self-contained because db.Open applies schema ensures
// without executing the numbered SQL migration files.
const durableTaskSchemaSQL = `
...
func (d *DB) ensureDurableTaskSchema(ctx context.Context) error {
	if d == nil || d.pool == nil {
		return nil
	}
	if _, err := d.pool.Exec(ctx, durableTaskSchemaSQL); err != nil {
		return fmt.Errorf("ensure durable task schema: %w", err)
	}
	return nil
}
