-- V368 rollback: restore the V364 tenant-scoped fingerprint index (no
-- credential dimension) and drop the credential_id column. Fails closed when
-- per-credential rows collide under the V364 identity.
BEGIN;
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'V368 down blocked: duplicate V364-identity provider_error_details rows require explicit operator resolution';
    END IF;
END
$$;
